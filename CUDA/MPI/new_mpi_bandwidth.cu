...
Date    : 19 June 2012
Compile : make new_mpi_bandwidth
Help    : mpirun -n <no of processes> -host <host ip> ./new_mpi_bandwidth -help
*/
...
void printResult(int myrank, char *myname, int device, char *devname, char *MODE, int SIZE, double h2d, double d2d, double d2h)
{
	printf("\n\
		Process %d : %s\n\
		Device %d : %s\n\ 
...
{
	int 		i, j, k;
	int 		comm_size, myrank, no_of_args, valid_args;
	...
	if(devcount > 0)
	{	
		recvbuf = (double *) malloc(devcount*sizeof(double));	
		...
		if(strcmp(MODE,"pageable") == 0)
		{
			for(i = myrank; i < devcount; i+=comm_size)
			{
				...
				if(device == i)
				{
					h2d = (double *) malloc((ceil((1.0*END-START)/STEP)+1)*sizeof(double));
					d2d = (double *) malloc((ceil((1.0*END-START)/STEP)+1)*sizeof(double));
					d2h = (double *) malloc((ceil((1.0*END-START)/STEP)+1)*sizeof(double));
					...
					for(SIZE=START ; SIZE<=END; SIZE+=STEP)			
					{
						...
						time = diff*1.0e-3;	
						h2d[j] = ( SIZE * sizeof(char) * 2.0 ) / ( 1024 * 1024 * time ) ;	
						/************************************** Host to Device Ends **************************************/
						...
						time = diff*1.0e-3;	
						d2d[j] = ( SIZE * sizeof(char) * 2.0 ) / ( 1024 * 1024 * time ) ;	
						...
						time = diff*1.0e-3;	
						d2h[j] = ( SIZE * sizeof(char) * 2.0 ) / ( 1024 * 1024 * time ) ;	
						/************************************** Device to Host Ends **************************************/	
					...
					for(SIZE=START, k=0 ; SIZE<=END; SIZE+=STEP,k++)			
						printResult(myrank,myname,device,devname,MODE,SIZE,h2d[k],d2d[k],d2h[k]);
