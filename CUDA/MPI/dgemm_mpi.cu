...
{
	int 		comm_size, myrank, i, no_of_args, valid_args, materr;
	int 		RowA, ColB, ColA;
				...
				if(device == i)
				{
					...
					if(materr=check(h_C,cpu_C,RowA,ColB)==-1)
						printf("\n\
							...
							\t colA : %d\n\
							Gflops\/s : %f\n",\
							myrank,myname,device,devname,MODE,RowA,ColB,ColA,gflops);
					else
		                  		printf("Error : CPU and GPU result do not match on Process:%d Device:%d\n\
							 index=%d\tdevice=%f\thost=%f\n",myrank,device,materr,h_C[materr],cpu_C[materr]);	
